...
│   ├── grasp_point_stable := grip-point drift since grasp onset >= GRASP_POINT_DRIFT_THRESHOLD=0.016 m for < GRASP_POINT_DRIFT_PERSISTENCE_FRAMES=2 consecutive raw frames  [predicates.py:2592-2657]
│   │       ├── _object_grip_point_in_object_frame(name)  [predicates.py:549-609] — leaf: centroid of finger/object contact points (env.sim.data.contact[i].pos) expressed in the object's own body frame; baseline latched at grasp onset, re-latched when the grasped object changes. Bound scaled to the Panda pad's 1.6 cm x 1.6 cm contact face (panda_gripper.xml:35,45), not chosen freely. Cumulative, so no debounce. No opinion on frames with no finger/object contact.
│   │       ├── _object_non_gripper_contact(name)  [predicates.py:611-644] — leaf: suspends the criterion while anything other than the gripper touches the object (the surface it is being placed on pushes it, shifting it in the fingers through no fault of the grasp). Baseline is re-latched when that contact clears, so the externally-imposed shift is forgiven rather than accumulated.
│   │       ├── 2-raw-frame debounce  [predicates.py:2645-2657] — deliberate exception to item 9's no-debounce direction: filters contact-position noise between adjacent raw frames (ep 2: 9 over-threshold frames in 8 runs, 7 of length 1). Counted in raw frames, NOT call_stride-scaled.
│   │       ⚠ Rolling contact deliberately out of scope: a rolled object migrates its grip point continuously without the grasp degrading, and is flagged here. Separating rolling from sliding needs the contact normal, which this predicate does not use.
│   ├── object_sync := _object_sync(name)                                  [predicates.py:1847-1863]  — REPLACED as object_grasped_safe's ongoing condition on 2026-09-01 (see CHANGES item 10); still computed and reported, but no longer gates grasp safety. Compares the object's body-origin velocity against the eef site's, so for an object whose centre sits r from the grasp, wrist rotation w injects |w x r| with no slip: measured 78/228 frames of a clean expert basket carry flagged (r ~ 0.18 m), against ~1.3-1.8 cm of actual grip-point migration.
...
│   └── object_grasped_safe := NOT object_released AND (object_grasped AND grasp_point_stable)  [predicates.py:2659-2668]  [`RELATIVE_SPEED_PERSISTENCE_FRAMES` / `GRASP_SAFE_GRACE_FRAMES` removed 2026-09-01, see cross-cutting note -- previously object_sync had its own false-frame grace, and object_grasped_safe had a further grace window on top of that]
