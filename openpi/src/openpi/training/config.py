...
except Exception as exc:  # pragma: no cover - allows serving without simulator deps.
    logging.warning("RoboCasa dataset registry unavailable at import time: %s", exc)
    DATASET_BASE_PATH = ""
...
class DataConfigFactory(abc.ABC):
    ...
    def _load_norm_stats(self, assets_dir: epath.Path, asset_id: str | None) -> dict[str, _transforms.NormStats] | None:
        ...
        try:
            ...
            # TODO: fix
            from openpi.groot_utils import groot_openpi_dataset as _groot_openpi_dataset
...
class LeRobotRobocasaDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # If checkpoint assets are missing, fall back to normalization stats embedded
        # in the local RoboCasa dataset directories.
        fallback_norm_stats = None
        if base.norm_stats is None and self.data_dirs:
            from openpi.groot_utils import groot_openpi_dataset as _groot_openpi_dataset

            try:
                if len(self.data_dirs) == 1:
                    norm_stats = _groot_openpi_dataset._load_norm_stats_from_groot_dataset(self.data_dirs[0])
                    if norm_stats is not None:
                        fallback_norm_stats = norm_stats
                        logging.info("Loaded norm stats from local data dir: %s", self.data_dirs[0])
                else:
                    norm_stats = _groot_openpi_dataset._load_norm_stats_from_groot_mixture_dataset(self.data_dirs)
                    if norm_stats is not None:
                        fallback_norm_stats = norm_stats
                        logging.info("Loaded combined norm stats from %d data dirs", len(self.data_dirs))
            except FileNotFoundError as exc:
                logging.info(
                    "Local RoboCasa norm stats unavailable; falling back to checkpoint assets if present: %s",
                    exc,
                )
