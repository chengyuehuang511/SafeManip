...
class RoboCasaGymEnv(gym.Env):
    ...
    def _copy_raw_obs(self, raw_obs):
        copied = {}
        for key, value in raw_obs.items():
            if isinstance(value, np.ndarray):
                copied[key] = np.copy(value)
            else:
    ...
    def get_observation(self, raw_obs):
        self._last_raw_obs = self._copy_raw_obs(raw_obs)
    ...
    def get_last_raw_observation(self):
        if self._last_raw_obs is None:
            return None
        return self._copy_raw_obs(self._last_raw_obs)
