...
class ArgsConfig:
    ...
    seed: int = 42
    """Random seed for model initialization, training, and dataset sampling."""
...
def main(config: ArgsConfig):
    ...
    if len(ds_soup_list) == 1:
        ...
        ds_weights = np.array(
            [np.power(len(dataset), config.ds_weights_alpha) for dataset in single_datasets]
        )
