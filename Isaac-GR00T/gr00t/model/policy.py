...
def _resolve_compute_dtype() -> torch.dtype:
    dtype_name = os.environ.get("GR00T_POLICY_COMPUTE_DTYPE", "bfloat16").lower()
    dtype_map = {
        "bf16": torch.bfloat16,
    ...
    if dtype_name not in dtype_map:
        raise ValueError(
            f"Unsupported GR00T_POLICY_COMPUTE_DTYPE={dtype_name!r}; "
...
class Gr00tPolicy(BasePolicy):
    ...
    def _get_action_from_normalized_input(self, normalized_input: Dict[str, Any]) -> torch.Tensor:
        disable_autocast = os.environ.get("GR00T_POLICY_DISABLE_AUTOCAST", "0") == "1"
        use_autocast = (
    ...
    def _load_model(self, model_path):
        ...
        model.to(device=self.device, dtype=COMPUTE_DTYPE)  # type: ignore
