#!/usr/bin/env python3
"""Single-task RoboCasa eval against the PRISTINE eval/models/grootn16
submodule (robocasa-benchmark leaderboard's "GR00T N1.6" submission fork of
NVIDIA Isaac GR00T N1.6), saving eval videos and (optionally) a replayable
rollout dataset in the same extras/ format used everywhere else in this repo.

Unlike the original Isaac-GR00T submodule, this fork already ships a
single-task, N-episode, in-process eval entry point --
`gr00t.eval.rollout_policy.run_gr00t_sim_policy` -- so no server thread or
custom rollout loop is needed here (`Gr00tPolicy`/`Gr00tSimPolicyWrapper`
load the checkpoint and run in-process). This script just:
  1. Monkeypatches `gymnasium.make` for the duration of the call (same
     technique as run_single_task_openpi.py) so the raw robocasa env
     `run_gr00t_sim_policy` creates internally (via `get_gym_env` ->
     `gym.make(f"robocasa/{task}", split=..., enable_render=True)`) gets
     wrapped with ReplayCapture + MultiCameraVideoCapture before any of
     grootn16's own wrappers (VideoRecordingWrapper/MultiStepWrapper) stack
     on top -- identical composition to every other model in this repo.
  2. Calls `run_gr00t_sim_policy(...)` completely unmodified.
Nothing inside eval/models/grootn16 or eval/simulators/robocasa is edited;
PYTHONPATH must point at eval/simulators/robocasa (not grootn16's own
external_dependencies/robocasa submodule, which is left uninitialized) so
this reuses the same pristine robocasa checkout as every other model.
"""
import argparse
import json
import sys
...
def run_single_task(
    *,
    ...
    if max_episode_steps is None:
        # Use robocasa's own per-task horizon (matches every other model in
        # this repo) rather than a flat default -- composite tasks need up
        # to ~4350 steps; a flat 720 would truncate them and unfairly
        # depress success rate relative to what the leaderboard reports.
    ...
    try:
        _, episode_successes, _ = run_gr00t_sim_policy(
            ...
            video_dir=None,  # our own 3-camera capture replaces grootn16's own video wrapper
            split=split,
        )
    finally:
